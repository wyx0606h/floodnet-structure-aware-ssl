...
|---|---|---|---|
| FloodNet | UAV RGB；常用公开划分 1445/450/448 | 受淹/未受淹建筑、道路及其他场景类别 | 主训练、主测试、全部核心结论 |
...
*Semi-Supervised Classification and Segmentation on High Resolution Aerial Images* 使用 398 张带标签和 1047 张无标签 FloodNet 训练图像。分割部分以 DeepLabV3+ 和 EfficientNet-B3 为基础，先使用人工标签训练，再生成伪掩码并逐渐提高伪标签损失权重。
...
为避免少数类别完全缺失，使用多标签分层抽样。每个比例生成三个固定种子，所有方法共享相同样本 ID。必须保存并版本化 split 文件。
...
2. Full Supervision：使用全部训练标签，仅作为性能上界。
...
| Method | 1% mIoU | 5% mIoU | 10% mIoU | 25% mIoU |
|---|---:|---:|---:|---:|
