...
|---|---|---|---|
| FloodNet Challenge Track 1 | UAV RGB；398 张公开掩码、1047 张无标签训练图像、450/448 张无公开掩码验证/测试图像 | 受淹/未受淹建筑、道路及其他场景类别 | 主训练与本地重划分评价、全部核心结论 |
...
当前持有的是 EARTHVISION 2021 Track 1 挑战版。七个 ZIP 是同一目录的下载分包，不是七个数据划分；合并后共有 2343 张 JPG 和 398 张 PNG 掩码。公开真值只覆盖 `Train/Labeled` 中的 398 张图像，官方 Validation 和 Test 不包含掩码，因此不能用于本地 mIoU、Boundary F1 或分类别 IoU。

为保证所有结论可复算，核心实验从 398 张公开真值中建立固定的本地 Train/Validation/Test 划分。官方 1047 张无标签训练图像只用于半监督训练；官方 Validation/Test 仅保留为未来挑战提交或无标签域观察，不进入本地模型选择和结果汇报。
...
| 目录 | 图像数 | 公开掩码数 | 本研究用途 |
|---|---:|---:|---|
| Train/Labeled/Flooded | 51 | 51 | 本地有监督池 |
| Train/Labeled/Non-Flooded | 347 | 347 | 本地有监督池 |
| Train/Unlabeled | 1047 | 0 | 无标签训练池 |
| Validation | 450 | 0 | 不用于本地定量评价 |
| Test | 448 | 0 | 不用于本地定量评价 |

七个 ZIP 合并后无重复路径，398 张有标签图像与掩码全部一一对应，掩码为 4000×3000 单通道 PNG，像素类别为 0–9。

### 14.2 本地固定评价协议

从 398 张公开真值中生成一个版本化的固定划分：

| 子集 | 图像数 | 用途 |
|---|---:|---|
| Local Train | 278 | 训练与少标注模拟 |
| Local Validation | 60 | 调参和 checkpoint 选择 |
| Local Test | 60 | 配置冻结后的最终本地评价 |

划分必须同时考虑整图 Flooded/Non-Flooded 标签、掩码中十类是否出现及各类像素占比，并检查连续航拍帧或近重复图像造成的场景泄漏。Local Test 一旦生成即固定，不随方法或随机种子改变。

### 14.3 少标注协议

少标注比例以 278 张 Local Train 为分母：

| 标注比例 | 近似有标签数 | 隐藏标签的本地训练图像 |
|---:|---:|---:|
| 5% | 14 | 264 |
| 10% | 28 | 250 |
| 25% | 70 | 208 |
...
主实验优先 5%、10%、25% 和 100%。每个比例生成三个多标签分层的有标签子集种子，所有方法共享相同样本 ID。隐藏标签的 Local Train 图像可作为额外无标签数据，但其真值只能用于离线伪标签质量分析，不能进入训练损失。1% 约为 3 张，难以覆盖关键类别，只作为可选极端压力测试，不列为最低投稿要求。

### 14.4 历史半监督协议

历史复现使用全部 398 张公开真值作为有标签池和 1047 张官方无标签图像，模型为 DeepLabV3+/EfficientNet-B3，训练逻辑为监督预热、生成伪标签和逐步增大伪标签损失。由于当前挑战包没有公开 Validation/Test 掩码，该复现主要验证训练逻辑和本地重划分结果，不声称精确复现论文的官方测试分数。
...
2. Local Full Supervision：使用 278 张 Local Train 标签，作为当前公开真值条件下的性能上界。
